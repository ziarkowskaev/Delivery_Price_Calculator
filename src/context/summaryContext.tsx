...
type SummaryAction = { type: "SET_SUMMARY"; payload: Summary };
...
export const useSummaryValue = (): Summary => {
  ...
  if (!context) {
    throw new Error(
      "useSummaryValue must be used within a SummaryContextProvider",
    );
  }
  return context[0];
};
...
export const useSummaryDispatch = (): Dispatch<SummaryAction> => {
  ...
  if (!context) {
    throw new Error(
      "useSummaryDispatch must be used within a SummaryContextProvider",
    );
  }
  return context[1];
};
